// API configuration and typed client with comprehensive error handling
...
export async function optimizeRoute(data: any): Promise<any> {
  try {
    return await postJSON(endpoints.optimize, data);
  } catch (error) {
    if (error instanceof ApiError) {
      throw error;
    }
